...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch total anggota
        const anggotaResponse = await fetch("http://localhost:3200/api/pengunjung/count");
        if (!anggotaResponse.ok) throw new Error("Gagal mengambil data total anggota");
        const anggotaData = await anggotaResponse.json();
        setTotalAnggota(anggotaData.count || 0);
